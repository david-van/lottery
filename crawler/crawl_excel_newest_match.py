...
def insert_excel(all_data, new_match_ids, new_match_dates):
    """
    将数据写入Excel文件
    
    Args:
        all_data (list): 包含比赛赔率数据的列表，每条记录对应一个比赛
        match_ids (list): 比赛的ID列表，与all_data一一对应
        match_dates (list): 比赛的日期列表，与all_data一一对应
    """
    # 创建一个新的工作簿
    wb = Workbook()
    ws = wb.active
    ...
    for i in range(len(all_data)):
        row = [all_data[i][-1][0], all_data[i][-1][1], new_match_dates[i], all_data[i][0][0], all_data[i][0][2], all_data[i][0][1]]
        crs = all_data[i][3]
        row.extend(crs)
        ttg = all_data[i][2]
        row.extend(ttg)
    ...
    # 保存Excel文件
    output_file = "12_9日可投注.xlsx"
    wb.save(output_file)
    print(f"数据已写入 {output_file}")


def crawl_insert_excel_newest_match():
    # 获取当前日期
    today = date.today()
    # 起始日期和结束日期
    ...
    # 这里插入数据到excel
    insert_excel(all_data, new_match_ids, new_match_dates)

if __name__ == "__main__":
    crawl_insert_excel_newest_match()
